...
Pong = {
  ...
  onkeydown: function(keyCode) {
    switch(keyCode) {
      ...
      case Game.KEY.THREE: this.takeOff();              break;
      ...
      case Game.KEY.UP:   if (!this.rightPaddle.auto) this.rightPaddle.moveUp();   break;
      case Game.KEY.DOWN: if (!this.rightPaddle.auto) this.rightPaddle.moveDown(); break;
    }
  },
  ...
  onkeyup: function(keyCode) {
    switch(keyCode) {
      ...
      case Game.KEY.UP: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingUp();   break;
      case Game.KEY.DOWN: if (!this.rightPaddle.auto) this.rightPaddle.stopMovingDown(); break;
    }
  },
  ...
  Paddle: {
    ...
    moveUp:         function() { 
      var paddle = this.x == 0 ? 'left' : 'right';
      if (this.up != 1) ss.rpc('pong.moveUp', paddle)
      this.up   = 1; 
    },
    moveDown:       function() { 
      var paddle = this.x == 0 ? 'left' : 'right';
      if (this.down != 1) ss.rpc('pong.moveDown', paddle) 
      this.down = 1; 
    },
    stopMovingUp:   function() { 
      var paddle = this.x == 0 ? 'left' : 'right';
      ss.rpc('pong.stopMovingUp', paddle)
      this.up   = 0; 
    },
    stopMovingDown: function() { 
      var paddle = this.x == 0 ? 'left' : 'right';
      ss.rpc('pong.stopMovingDown', paddle)
      this.down = 0; 
    }
  ...
  Ball: {
    ...
    update: function(dt, leftPaddle, rightPaddle) {
      ...
      ss.rpc('pong.ball', this.x, this.y);
